...
__global__ void __launch_bounds__(256, 4) compute_off_diag_d_ij_and_alpha_i_kernel(
    ...
    const Number* __restrict__ speed_of_sound,
    Number* __restrict__ alpha_i,
    Number* __restrict__ dij_matrix,
    ...
    int n_dofs)
{
    ...
    U_i_local[0] = __ldg(&U.rho[i]);
    U_i_local[1] = __ldg(&U.momentum_x[i]);
    if constexpr (dim >= 2) U_i_local[2] = __ldg(&U.momentum_y[i]);
    if constexpr (dim == 3) U_i_local[3] = __ldg(&U.momentum_z[i]);
    U_i_local[dim + 1] = __ldg(&U.energy[i]);
    ...
    for (int idx = row_start; idx < row_end; ++idx) {
        ...
        #pragma unroll
        for (int d = 0; d < dim; ++d) {
            c_ij[d] = __ldg(&cij_matrix.values[idx * dim + d]);
        }

        Number U_j_local[dim + 2];
        U_j_local[0] = __ldg(&U.rho[j]);
        U_j_local[1] = __ldg(&U.momentum_x[j]);
        if constexpr (dim >= 2) U_j_local[2] = __ldg(&U.momentum_y[j]);
        if constexpr (dim == 3) U_j_local[3] = __ldg(&U.momentum_z[j]);
        U_j_local[dim + 1] = __ldg(&U.energy[j]);
    ...
    const Number mass = __ldg(&mi_matrix.values[i]);
...
__global__ void __launch_bounds__(192, 4) compute_limiter_kernel(
    ...
    Number* __restrict__ lij_matrix,
    const Number* __restrict__ bounds,
    const Sparsity& sparsity,
    const MijMatrix<Number>& mij_matrix,
    const MiMatrixInverse<Number>& mi_inv_matrix,
    Number tau,
    int n_dofs)
{
    ...
    const Number m_i_inv = __ldg(&mi_inv_matrix.values[i]);

    Number U_i_new_array[dim + 2];
    U_i_new_array[0] = __ldg(&new_U.rho[i]);
    U_i_new_array[1] = __ldg(&new_U.momentum_x[i]);
    if constexpr (dim >= 2) U_i_new_array[2] = __ldg(&new_U.momentum_y[i]);
    if constexpr (dim == 3) U_i_new_array[3] = __ldg(&new_U.momentum_z[i]);
    U_i_new_array[dim + 1] = __ldg(&new_U.energy[i]);

    Number F_iH_array[dim + 2];
    F_iH_array[0] = __ldg(&ri.r_rho[i]);
    F_iH_array[1] = __ldg(&ri.r_momentum_x[i]);
    if constexpr (dim >= 2) F_iH_array[2] = __ldg(&ri.r_momentum_y[i]);
    if constexpr (dim == 3) F_iH_array[3] = __ldg(&ri.r_momentum_z[i]);
    F_iH_array[dim + 1] = __ldg(&ri.r_energy[i]);
    ...
    bounds_i.rho_min = __ldg(&bounds[i * 3 + 0]);
    bounds_i.rho_max = __ldg(&bounds[i * 3 + 1]);
    bounds_i.s_min = __ldg(&bounds[i * 3 + 2]);
    ...
    for (int col_idx = 1; col_idx < row_length; ++col_idx) {
        ...
        const unsigned int j = __ldg(&sparsity.col_indices[idx]);

        Number F_jH_array[dim + 2];
        F_jH_array[0] = __ldg(&ri.r_rho[j]);
        F_jH_array[1] = __ldg(&ri.r_momentum_x[j]);
        if constexpr (dim >= 2) F_jH_array[2] = __ldg(&ri.r_momentum_y[j]);
        if constexpr (dim == 3) F_jH_array[3] = __ldg(&ri.r_momentum_z[j]);
        F_jH_array[dim + 1] = __ldg(&ri.r_energy[j]);

        const Number m_j_inv = __ldg(&mi_inv_matrix.values[j]);
        const Number m_ij = __ldg(&mij_matrix.values[idx]);
...
__global__ void __launch_bounds__(256, 4) high_order_update_iter1_kernel(
    ...
    const Number* __restrict__ lij_matrix,
    Number* __restrict__ lij_next,
    const Number* __restrict__ bounds,
    const Sparsity& sparsity,
    int n_dofs)
{
    ...
    for (int col_idx = 1; col_idx < row_length; ++col_idx) {
        ...
        Number l_ij_ = __ldg(&lij_matrix[idx]);
        const int idx_ji = __ldg(&sparsity.transpose_indices[idx]);
        if (idx_ji >= 0) {
            l_ij_ = fmin(l_ij_, __ldg(&lij_matrix[idx_ji]));
        }

        Number P_ij[dim + 2];
        P_ij[0] = __ldg(&pij_matrix.p_rho[idx]);
        P_ij[1] = __ldg(&pij_matrix.p_momentum_x[idx]);
        if constexpr (dim >= 2) P_ij[2] = __ldg(&pij_matrix.p_momentum_y[idx]);
        if constexpr (dim == 3) P_ij[3] = __ldg(&pij_matrix.p_momentum_z[idx]);
        P_ij[dim + 1] = __ldg(&pij_matrix.p_energy[idx]);
