...
public:
          ...
          parabolic_solver(n_dofs, _mu, _cv_inverse_kappa, _stream)
    {
        ...
        std::cout << "\nComputing transpose indices" << std::endl;
    ...
    VTUOutput<dim>* output_handler)
{
    ...
    const Number_cu u_inf = vel_mag * static_cast<Number_cu>(config.direction[0]);
    const Number_cu v_inf = vel_mag * static_cast<Number_cu>(config.direction[1]);
    const Number_cu w_inf = vel_mag * static_cast<Number_cu>(config.direction[2]);

    Number_cu E_inf;
    if constexpr (dim == 2) {
        E_inf = p_inf / (gamma - Number_cu(1)) +
                Number_cu(0.5) * rho_inf * (u_inf * u_inf + v_inf * v_inf);
    } else {
        E_inf = p_inf / (gamma - Number_cu(1)) +
                Number_cu(0.5) * rho_inf * (u_inf * u_inf + v_inf * v_inf + w_inf * w_inf);
    }
